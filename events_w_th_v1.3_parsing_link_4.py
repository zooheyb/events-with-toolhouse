...
def parse_luma(start_date, end_date):
    ...
    for link in full_links:
        ...
        # Here, you can extract the title and other information as needed
        title = title_elements[0].text.strip() if title_elements else "No Title Found"
        
        # Extract event date and time from the content
        content_divs = event_soup.find_all('div')
        for div in content_divs:
            if "October 28th" in div.text:  # Example search for the specific date
                # Parse the time range from the content
                time_text = div.text.split('from')[-1].strip() if 'from' in div.text else "Time not found"
                events.append({
                    'title': title,
                    'link': link,
                    'date': 'October 28, 2024',  # Hardcoded for example; extract dynamically if needed
                    'time': time_text
                })
